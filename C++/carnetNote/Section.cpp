...
void Section::gerer()
{
				...
				do
				{
								...
								cout<<"tapez 7 pour afficher les notes et la moyenne d'un eleve par matiere"<<endl;
								cout<<"tapez 8 pour afficher les notes et la moyenne d'un eleve par matiere Corriger" << endl;
								...
								switch(choix)
								{
												...
												case 6:preparerBulletins();
															 ...
															 break;
												case 7:affiNoteMoyenneEleve();
													break;
												case 8:preparerBulletins();affiNoteMoyenneEleveCorriger();
...
//taf evaluation
void Section::affiNoteMoyenneEleve()
{
	string choixEleve;//variable permettant de connaitre le choix de l'utilisateur
	int verifEleve=0;//variable permettant de savoir si l'élève existe ou pas
	cout << "entrez le nom de l'élève dont vous souhaitez voire les notes" << endl;
	cin >> choixEleve;
	//on boncle pour tout les élèves de la section
	for(int noEleves=0;noEleves<vectEtudiants.size();noEleves++)
	{
		//si le nom entrez par l'utilisateur correspond à un nom du vecteur
		if (choixEleve==vectEtudiants[noEleves].getNom())
		{
			//on boucle pour toute les matières
			for(int noMat=0;noMat<vectMatieresSection.size();noMat++)
			{
			float total=0;
			int totalNote=0;
				//on affiche le nom de la matière
				cout << vectMatieresSection[noMat]->getNomMatiere() << ":" << endl;
				//on boncle pour toute les évaluations
				for(int noEval=0;noEval<vectEvaluations.size();noEval++)
				{
					vector<Note> vectNoteMat=vectEvaluations[noEval].getVectNotes();
					//si la matiere de l'évaluation correspond a la matiere que l'on traite
					if(vectEvaluations[noEval].getLaMatiere()->getNomMatiere()==vectMatieresSection[noMat]->getNomMatiere())
					{
						//boucle pour toute les notes d'une évaluation
						for(int noNote=0;noNote<vectNoteMat.size();noNote++)
							//si l'élève du vecteur correspond a celui selectionner
							if(vectNoteMat[noNote].getLEtudiant()->getNom()==vectEtudiants[noEleves].getNom())
							{
								totalNote++;//ajoute un au total e note de la matiere
								cout <<"note "<<totalNote<<": "<< vectNoteMat[noNote].getValeur()<< endl;
								total+=vectNoteMat[noNote].getValeur();//ajoute la note au total
							}
					}
				}
			//si une note à été entrer
			if(total!=0)
			{
				float moyenne=total/totalNote;//calcule la moyenne
				cout << "moyenne: "<<moyenne<<endl;
			}
			else cout << "aucune évaluation entrer pour cette matière"<<endl;	
			}
			verifEleve =1;//on met la valeur de la variable qui permet de verifier si un élèves existe car il existe un élève
		}
	}
	//si aucun élève correspond au nom entrer
	if (verifEleve==0)
	{
		cout << "eleve n'existe pas" << endl;
	}
}

void Section::affiNoteMoyenneEleveCorriger()
{
Etudiant* pMonEleve;
int noEleve = 0;
string choixEleve;
cout << "entrez le nom de l'élève dont vous souhaitez voire les notes" << endl;
getline(cin,choixEleve);
	while (!(noEleve==vectEtudiants.size() || vectEtudiants[noEleve].getNom()==choixEleve))
	{
		noEleve++;
	}
	if(noEleve==vectEtudiants.size())
	{
		cout << "élève pas trouvé" << endl;
	}
	else
	{
	  pMonEleve=&vectEtudiants[noEleve];
		if(mapNotesParEtudiantEtParMatiere.count(pMonEleve)==1)
		{
	  	...
	  	map<Matiere*,vector<Note*> >::iterator itMatiere;
			for(itMatiere=saMap.begin();itMatiere!=saMap.end();++itMatiere)
			{
				cout << itMatiere->first->getNomMatiere() << endl;
				vector<Note*> vectNoteEleve=itMatiere->second;
				for(int noNote=0;noNote<vectNoteEleve.size();noNote++)
				{
					cout << "note" << " "<< noNote+1getNomMatiere ( )   {
	return nomMatiere;
 << ": "<< vectNoteEleve[noNote]->getValeur() << endl;
					total+=vectNoteEleve[noNote]->getValeur();
				}
				cout << "moyenne: " << total/vectNoteEleve.size() << endl;
			}					
		}
		else
		{
			cout << "cet élève n'a fait aucune évaluation" << endl;
		}
	}
}
